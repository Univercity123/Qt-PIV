...
class CameraThread(QThread):
    ...
    def __init__(self, camera_index=0):
        ...
        self.h5_writer = None   # HDF5文件对象
        self.recorded_flows = []  # 存储记录的光流数据
        self.record_start_ts = []  # 记录开始时间戳
        self.record_end_ts = []    # 记录结束时间戳
        self.last_processing_ts = 0  # 最后处理时间戳
        self.width = 0  # 添加宽度属性
        self.height = 0  # 添加高度属性
    ...
    def start_recording(self, filename):
        ...
        try:
            ...
            # 创建文件路径，将文件保存在 output 文件夹中
            filename = os.path.join(output_dir, filename)
            
            # 创建HDF5文件
            self.h5_writer = h5py.File(filename, "w")
            
            # 创建数据集
            self.h5_writer.create_dataset("flow_start_ts", (0,), maxshape=(None,), dtype=np.int64)
            self.h5_writer.create_dataset("flow_end_ts", (0,), maxshape=(None,), dtype=np.int64)
            self.h5_writer.create_dataset("flow", (0, 2, self.height, self.width), 
                                        maxshape=(None, 2, self.height, self.width), 
                                        dtype=np.float32, compression="gzip")
            
            # 添加元数据
            self.h5_writer["flow"].attrs["input_file_name"] = "live_camera"
            self.h5_writer["flow"].attrs["checkpoint_path"] = "metavision_dense_optical_flow"
            self.h5_writer["flow"].attrs["event_input_height"] = self.height
            self.h5_writer["flow"].attrs["event_input_width"] = self.width
            self.h5_writer["flow"].attrs["delta_t"] = self.dt_step
            
            # 重置记录缓冲区
            self.recorded_flows = []
            self.record_start_ts = []
            self.record_end_ts = []
    ...
    def stop_recording(self):
        """停止记录并保存H5文件"""
        if not self.recording or not self.h5_writer:
            return False
            
        try:
            # 如果有未写入的数据，先写入
            if self.recorded_flows:
                self.write_recorded_data()
    ...
    def record_flow_data(self, processing_ts, flow_buffer):
        """记录当前光流数据"""
        if not self.recording or not self.h5_writer:
            return
        ...
        try:
            ...
            # 创建稠密光流场
            flow_x = np.zeros((self.height, self.width), dtype=np.float32)
            flow_y = np.zeros((self.height, self.width), dtype=np.float32)
            ...
            # 将光流数据添加到记录列表
            dense_flow = np.stack([flow_x, flow_y])
            self.recorded_flows.append(dense_flow)
            self.record_start_ts.append(processing_ts - self.dt_step)
            self.record_end_ts.append(processing_ts)
            
            # 定期写入数据（每10帧写入一次）
            if len(self.recorded_flows) >= 10:
                self.write_recorded_data()
    ...
    def write_recorded_data(self):
        """将记录的数据写入HDF5文件"""
        if not self.recorded_flows or not self.h5_writer:
            return
            
        try:
            # 获取当前数据集大小
            current_size = self.h5_writer["flow"].shape[0]
            new_size = current_size + len(self.recorded_flows)
            
            # 调整数据集大小
            self.h5_writer["flow"].resize((new_size, 2, self.height, self.width))
            self.h5_writer["flow_start_ts"].resize((new_size,))
            self.h5_writer["flow_end_ts"].resize((new_size,))
            
            # 写入新数据
            self.h5_writer["flow"][current_size:new_size] = np.stack(self.recorded_flows)
            self.h5_writer["flow_start_ts"][current_size:new_size] = np.array(self.record_start_ts)
            self.h5_writer["flow_end_ts"][current_size:new_size] = np.array(self.record_end_ts)
    ...
    def run(self):
        ...
        try:
            ...
            # 获取摄像头分辨率并保存
            self.width = slicer.camera().width()
            self.height = slicer.camera().height()
            
            # 初始化光流算法
            self.init_flow_algorithm(self.width, self.height)
            
            # 创建光流可视化图像缓冲区
            flow_output_img = np.zeros((self.height, self.width, 3), np.uint8)
            combined_output_img = np.zeros((self.height, self.width, 3), np.uint8)
            
            # 性能优化：使用固定缓冲区减少内存分配
            frame_buffer = np.zeros((self.height, self.width, 3), np.uint8)

            # 设置初始处理时间戳
            self.last_processing_ts = int(time.time() * 1000000)  # 转换为微秒
            ...
            while self.running:
                ...
                for slice in slicer:
                    ...
                    # 更新处理时间戳
                    processing_ts = self.last_processing_ts + self.dt_step
                    self.last_processing_ts = processing_ts
                    
                    # 记录光流数据（如果正在记录）
                    if self.recording:
                        self.record_flow_data(processing_ts, self.flow_buffer)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # 连接保存按钮信号
        self.ui.action.triggered.connect(self.start_saving)
        self.ui.action_2.triggered.connect(self.stop_saving)

    def start_saving(self):
        """开始保存光流数据到H5文件"""
        if self.recording:
            self.ui.statusbar.showMessage("已经在记录中")
            return
            
        # 生成文件名
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        self.recording_filename = f"flow_recording_{timestamp}.h5"
        
        # 开始记录
        if self.camera_thread.start_recording(self.recording_filename):
            self.recording = True
            self.ui.statusbar.showMessage(f"开始记录光流到 {self.recording_filename}")
        else:
            self.ui.statusbar.showMessage("无法开始记录")
    
    def stop_saving(self):
        """停止保存光流数据"""
        if not self.recording:
            self.ui.statusbar.showMessage("没有在记录中")
            return
            
        if self.camera_thread.stop_recording():
            self.recording = False
            self.ui.statusbar.showMessage(f"已停止记录，数据保存到 {self.recording_filename}")
        else:
            self.ui.statusbar.showMessage("停止记录时出错")
